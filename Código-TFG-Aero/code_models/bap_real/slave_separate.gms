...
of_generate.. z=e=30000+sum((f1,f2)$connec(f1,f2),conex(f1,f2)*(cost_connec(f1,f2)+200)) - sum(F,Selected_F(F) * dual(F)) + sum((F,c1,c2,t1,t2)$(Flight(F, c1, c2, t1, t2) and not(Base(c1))) , First_F(F)*1000) + sum((F,c1,c2,t1,t2)$(Flight(F, c1, c2, t1, t2) and not(Base(c2))), Last_F(F)*5000);
