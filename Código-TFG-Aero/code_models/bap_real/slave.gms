...
cost_new_LOF.. cost_duty=e=30000+sum((f1,f2)$connec(f1,f2),conex(f1,f2)*(cost_connec(f1,f2)+200))+sum((F,c1,c2,t1,t2)$Flight(F, c1, c2, t1, t2), Price_comb(c1)*reposted_combustible(F)) + sum((F,c1,c2,t1,t2)$(Flight(F, c1, c2, t1, t2) and not(Base(c1))) , First_F(F)*1000) + sum((F,c1,c2,t1,t2)$(Flight(F, c1, c2, t1, t2) and not(Base(c2))), Last_F(F)*5000)  ;
...
capacity2(F).. reposted_combustible(F)+ sum(f1$connec(f1,F), comb_aircraft(f1))=l=6000;
...
inicio12(F).. comb_aircraft(F)=l=6000*Selected_F(F);
