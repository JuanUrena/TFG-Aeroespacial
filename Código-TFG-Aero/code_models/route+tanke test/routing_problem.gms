...
of.. FinalCost =e=  TotalCost- r*400;

res_cost.. TotalCost=e=sum(F,First_F(F)*600)+sum((f1,f2)$connec(f1,f2),use_conec(f1,f2)*(cost_connec(f1,f2)+20)) + sum((F,c1,c2,t1,t2)$(Flight(F, c1, c2, t1, t2) and not(Base(c1))) , First_F(F)*20) + sum((F,c1,c2,t1,t2)$(Flight(F, c1, c2, t1, t2) and not(Base(c2))), Last_F(F)*50);
